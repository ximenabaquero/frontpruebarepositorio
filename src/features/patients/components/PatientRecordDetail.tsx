    ...
    try {
      const res = await fetch(
        `${apiBaseUrl}/api/v1/medical-evaluations/${evaluationId}/confirmar`,
        {
          ...
          body: JSON.stringify({ terms_accepted: true, patient_signature: signature }),
        },
      );
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
              ...
              <div>
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm text-gray-600 space-y-2 max-h-44 overflow-y-auto">
                  <p>
                    Al confirmar esta valoración, la paciente declara que ha leído, comprendido y
                    acepta el registro clínico elaborado por el profesional de Cold Esthetic.
                  </p>
                  <p>
                    La paciente confirma que la información personal y clínica suministrada es
                    verídica y autoriza su uso con fines estrictamente médicos y de seguimiento.
                  </p>
                  <p>
                    La paciente entiende que los procedimientos descritos en este documento se
                    realizarán bajo su consentimiento informado y que puede contactar al profesional
                    ante cualquier duda o complicación posterior.
                  </p>
                  <p>
                    Este documento tiene validez legal como constancia de aceptación del tratamiento
                    y del plan de procedimientos acordado.
                  </p>
                </div>
              </div>
        ...
        >
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "32px" }}>
            ...
            <div style={{ textAlign: "right" }}>
              <p style={{ fontSize: "10px", color: "#9ca3af", textTransform: "uppercase", marginBottom: "2px" }}>Factura N°</p>
              <p style={{ fontSize: "18px", fontWeight: 700, color: "#1f2937" }}>
                #{String(evaluationId).padStart(5, "0")}
              </p>
              <p style={{ fontSize: "11px", color: "#6b7280", marginTop: "4px" }}>
                {evaluation?.created_at
                  ? new Date(evaluation.created_at).toLocaleDateString("es-CO", { day: "2-digit", month: "long", year: "numeric" })
                  : ""}
              </p>
            </div>
          </div>
          ...
          <div style={{ marginBottom: "28px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", background: "#f9fafb", borderRadius: "10px", padding: "16px", fontSize: "12px" }}>
              <div>
                ...
                <p style={{ fontWeight: 600, color: "#1f2937" }}>{evaluation?.patient?.first_name} {evaluation?.patient?.last_name}</p>
              </div>
              <div>
            ...
            <div style={{ textAlign: "right" }}>
              <div style={{ borderTop: "2px solid #e5e7eb", paddingTop: "12px" }}>
                ...
                <p style={{ fontSize: "32px", fontWeight: 800, color: "#059669" }}>
                  ${evaluation?.procedures?.reduce((sum: number, proc: any) => sum + Number(proc.total_amount), 0).toLocaleString("es-CO")}
                </p>
              </div>
            </div>
          </div>
