        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 space-y-4">
            <div className="flex items-center justify-between px-2">
              ...
              <span className="text-[10px] font-bold text-slate-400 tracking-[0.2em]">MONITOREO TIEMPO REAL</span>
            </div>
            ...
            <div className="grid gap-2">
              {ALERTS.map((alert) => (
                <Link 
                  ...
                  className="group flex items-center justify-between p-4 bg-white ring-1 ring-slate-200 rounded-xl hover:ring-indigo-500 transition-all duration-200 shadow-sm"
                >
                  <div className="flex items-center gap-5 min-w-0">
                    <div className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${alert.level === 'critical' ? 'bg-rose-500/10 text-rose-700' : 'bg-amber-500/10 text-amber-700'}`}>
                      <AlertTriangle className="w-5 h-5" />
                    </div>
                    <div className="truncate">
                      <p className="text-base font-semibold text-slate-900 group-hover:text-indigo-700 transition-colors">{alert.patient}</p>
                      <p className="text-sm text-slate-500 font-medium">{alert.msg}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 ml-4">
                    <span className="hidden md:block text-[10px] font-bold text-slate-400 group-hover:text-indigo-600 uppercase tracking-widest transition-colors">Expediente</span>
